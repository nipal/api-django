...
class LegacyEventSerializer(
    ...
    tags = RelatedLabelField(
        queryset=models.EventTag.objects.all(), many=True, required=False
    )
    ...
    def get_is_organizer(self, obj):
        if not (
            hasattr(self.context["request"].user, "type")
            and self.context["request"].user.type == Role.PERSON_ROLE
        ):
            return False
            ...
            if organizers is not None:
                ...
                for organizer in organizers:
                    models.OrganizerConfig.objects.create(
                        event=instance, person=organizer
                    )
...
class CalendarSerializer(serializers.HyperlinkedModelSerializer):
    class Meta:
        ...
        extra_kwargs = {"url": {"view_name": "legacy:calendar-detail"}}
...
class EventTagSerializer(serializers.HyperlinkedModelSerializer):
    class Meta:
        ...
        extra_kwargs = {"url": {"view_name": "legacy:eventtag-detail"}}
...
class EventRSVPListSerializer(UpdatableListSerializer):
    ...
    def get_additional_fields(self):
        return {"event_id": self.context["event"]}
