...
from agir.people.views.views_mixins import (
    NavsProfilMixin,
    NAVS_PROFILE_PREFERENCE,
    NAVS_PROFILE_PARTICIPATION,
    NAVS_PROFILE_IDENTITY,
    NAVS_PROFILE_CONTACT,
    NAVS_PROFILE_SKILLS,
    NAVS_PROFILE_ACT,
    NAVS_PROFILE_PRIVACY,
)
...
class DeleteAccountView(HardLoginRequiredMixin, NavsProfilMixin, DeleteView):
    template_name = "people/delete_account.html"

    def get_context_data(self, **kwargs):
        return super().get_context_data(tab_code=NAVS_PROFILE_PRIVACY)
...
class ChangeProfilPreference(SoftLoginRequiredMixin, NavsProfilMixin, UpdateView):
    template_name = "people/information_personal.html"
    form_class = PreferencesForm
    success_url = reverse_lazy("confirmation_profile")

    def get_context_data(self, **kwargs):
        return super().get_context_data(tab_code=NAVS_PROFILE_PREFERENCE)
...
class ChangeProfileParticipation(SoftLoginRequiredMixin, NavsProfilMixin, UpdateView):
    template_name = "people/information_personal.html"
    form_class = ParticipationForm
    success_url = reverse_lazy("confirmation_profile")

    def get_context_data(self, **kwargs):
        return super().get_context_data(tab_code=NAVS_PROFILE_PARTICIPATION)
...
#
# class ChangeProfilePrivacy(SoftLoginRequiredMixin, NavsProfilMixin, UpdateView):
#     template_name = "people/information_personal.html"
...
class ChangeProfilePersoView(SoftLoginRequiredMixin, NavsProfilMixin, UpdateView):
    template_name = "people/information_personal.html"
    form_class = InformationPersonalForm
    success_url = reverse_lazy("confirmation_profile")

    def get_context_data(self, **kwargs):
        return super().get_context_data(tab_code=NAVS_PROFILE_IDENTITY)
...
class ChangeProfileContactView(SoftLoginRequiredMixin, NavsProfilMixin, UpdateView):
    ...
    def merge_account(self):
        try:
            email_to_merge = self.request.POST.get("email_merge_target")
            user_to_merge = Person.objects.get(email=email_to_merge)
        except Person.DoesNotExist:
            # add error message
            return

        if user_to_merge.pk == self.person.pk:
            # add error message "It is yourself"
            return

        merge_persons(self.person, user_to_merge)
...
class ChangeProfileSkillsView(SoftLoginRequiredMixin, NavsProfilMixin, UpdateView):
    template_name = "people/information_skills.html"
    form_class = ActivityAblebilityForm
    success_url = reverse_lazy("confirmation_profile")

    def get_context_data(self, **kwargs):
        return super().get_context_data(tab_code=NAVS_PROFILE_SKILLS)
