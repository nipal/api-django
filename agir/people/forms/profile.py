                ...
                Fieldset(
                    ...
                    Row(
                        HalfCol(
                            ...
                            Row(
                                HalfCol(description_gender),
                                HalfCol(description_birth_date),
                            ),
                        ),
                        HalfCol(
                            Row(
                                FullCol(
                                    Field(
                                        "location_address1", placeholder=_("1ère ligne")
                                    )
                                ),
                                FullCol(
                                    Field(
                                        "location_address2", placeholder=_("2ème ligne")
                                    )
                                ),
...
class InformationConfidentialityForm(Form):
    def get_fields(self, fields=None):
        ...
        block_template = """
                    ...
                    <div class="controls">
                      <div>{value}</div>
                      <p class="help-block">{help_text} <a href="{url_contact}">{link_text}</a></p>
                    </div>
                """
        ...
        fields.extend([Fieldset("Données personnelles", validation_block)])
        return fields
...
class InformationContactForm(MetaFieldsMixin, ContactPhoneNumberMixin, forms.ModelForm):
    ...
    email_add_merge = forms.EmailField(
        max_length=200,
        label=_("ajouter une adresse e-mail"),
        required=False,
        help_text="Ce champ permet d'associer une adresse e-mail a votre compte. ",
    )
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.helper = FormHelper()
        self.helper.form_method = "POST"
        self.helper.add_input(Submit("submit", "Enregistrer mon numérot"))
                    ...
                    Fieldset(
                        ...
                        Row(
                            ThirdCol(
                                ...
                                Field(
                                    "party_responsibility", placeholder="Responsabilité"
                                ),
                                "mandates",
                            ),
                            ThirdCol(
...
class PreferencesForm(TagMixin, forms.ModelForm):
    tags = [
        (
            "newsletter_efi",
            _(
    ...
    def get_fields(self, fields=None):
        ...
        fields.extend(
            [
                Fieldset("Préférences de sms", "subscribed_sms"),
                Fieldset(
